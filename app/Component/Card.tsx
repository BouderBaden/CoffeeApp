...
export default function Card({ item  }: CardProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 relative z-[50] flex flex-col justify-between  gap-2">
      ...
      <p className="text-[12px] text-gray-900 flex items-center gap-2"><span className="font-bold flex items-center gap-1"><BiSolidCoffeeBean/> Puissance:</span><span>{item.strengh}</span></p>
      ...
      <button onClick={handleAddToCart}  className="self-end bg-orange-400 transition ease-in-out hover:bg-orange-600 rounded-md text-white p-2	">Ajouter au panier</button>
    </div>
  );
}