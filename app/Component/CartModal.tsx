      ...
      {cartModalOpen && (
        <div className="w-[300px] h-[100vh] overflow-y-auto fixed top-[50px] right-0 bg-white border border-l-gray-700 pb-16">
          ...
          <div className="p-4">
            ...
            {products.length === 0 ? (
              ...
              products.map((product) => (
                <div key={product.id} className="border-b py-2">
                  ...
                  <button className="mt-2 w-6 h-6 bg-gray-500 transition ease-in-out hover:bg-gray-600 p-2 rounded-lg cursor-pointer flex items-center justify-center"  onClick={() => decrementQuantity(product.id)}>
                    -
                  </button>
                  <button className="mt-2 w-6 h-6 bg-gray-500 transition ease-in-out hover:bg-gray-600 p-2 rounded-lg cursor-pointer flex items-center justify-center"  onClick={() => incrementQuantity(product.id)}>
                    +
                  </button>
                  <button className="mt-2 w-6 h-6 bg-red-500 transition ease-in-out hover:bg-red-600 p-2 rounded-lg cursor-pointer flex items-center justify-center"  onClick={() => removeFromCart(product.id)}>
                    <FaTrash />
                  </button>
                  </div>
            ...
            <button className="mt-5 self-end bg-orange-400 transition ease-in-out hover:bg-orange-600 rounded-md text-white p-2	">Proceder au paiement</button>
          </div>
        </div>
      )}
    </>
